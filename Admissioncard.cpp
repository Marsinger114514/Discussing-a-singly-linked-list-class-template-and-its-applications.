...
void School:: Display(std::ostream &out) const					//展示考生信息
{
	out << "该校考生信息：" << std::endl;
	link.PutList(out);                                     
}
...
void School::Show(ostream &out) const                            //输出学校名称
{
	out << "学校名称：" << schoolname << endl;
}

void School::SortCards(const Card &c,bool ascending_num) {       //排序准考证结点

	Card y(c);
	link.Sort(y,ascending_num);
}

ostream & operator<<(ostream &out, const School &s)				//输出链表
{
	s.Show(out);
